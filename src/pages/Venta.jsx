...
export default function Venta({ cashDrawer, onCashClosed }) {
  ...
  // ---------------------------------------------------------
  // 🕵️‍♂️ LÓGICA DE FUERZA BRUTA (REGEX) PARA EL NOMBRE
  // ---------------------------------------------------------
  const nombreCajero = useMemo(() => {
    // 1. Obtener la cadena cruda de donde sea
    let raw = cashDrawer?.user_full_name || localStorage.getItem('user') || localStorage.getItem('user_data') || "";
    
    // Convertimos a string por si acaso viene como objeto
    let str = typeof raw === 'object' ? JSON.stringify(raw) : String(raw);

    // 2. INTENTO CON "TIJERA" (REGEX) - Busca exactamente el patrón "FULL_NAME":"ALGO"
    const matchFull = str.match(/"FULL_NAME":"([^"]+)"/);
    if (matchFull) return matchFull[1]; // Retorna "CAJA PRINCIPAL"

    const matchUser = str.match(/"USERNAME":"([^"]+)"/);
    if (matchUser) return matchUser[1]; // Retorna "CAJA"

    const matchName = str.match(/"name":"([^"]+)"/i);
    if (matchName) return matchName[1];

    // 3. Si no tiene formato raro, devolvemos el texto limpio
      ...
      <div style={{ flex: 1, padding: "30px", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "15px" }}>
          {filteredProducts.map(p => (
            <button key={p.id} onClick={() => addProduct(p)} disabled={getAvailableStock(p) <= 0} style={{ background: "#111", border: "1px solid #222", padding: "15px", borderRadius: "12px", textAlign: "left", cursor: "pointer" }}>
